...
const ARCHITECTURE_ENDPOINTS: Record<string, string> = {
  ...
  'routing': `${AGENT_ENDPOINT.replace('/api/pydantic-agent', '')}/api/agent-routing`,
  'parallel': `${AGENT_ENDPOINT.replace('/api/pydantic-agent', '')}/api/agent-parallel`,
  'supervisor': `${AGENT_ENDPOINT.replace('/api/pydantic-agent', '')}/api/agent-supervisor`,
  'guardrail': `${AGENT_ENDPOINT.replace('/api/pydantic-agent', '')}/api/agent-guardrail`,
};

export const getArchitectureEndpoint = (architecture: string): string => {
  return ARCHITECTURE_ENDPOINTS[architecture] || AGENT_ENDPOINT;
};
...
export const sendMessage = async (
  ...
  try {
    ...
    const endpoint = getArchitectureEndpoint(architecture);
